...
namespace SkladApp.Views
{
    public partial class AdminWindow : Window
    {
        ...
        public AdminWindow(User currentUser)
        {
            ...
            UserInfoTextBlock.Text = $"User: {_currentUser.ФИО} (Administrator)";

            _dataGrids = new Dictionary<string, DataGrid>
            {
                { "Пользователь", UsersDataGrid },
                ...
                if (!string.IsNullOrEmpty(tableName))
                {
                    object record = tableName switch
                    {
                        "Пользователь" => new User
                        {
                            ...
                            Роль = rowView["Роль"].ToString(),
                            Логин = rowView["Логин"].ToString(),
                            Пароль = rowView["Пароль"].ToString(),
